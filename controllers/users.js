...
const addUser = async (req, res) => {
  ...
  try {
    ...
    if (!npp || !nickname || !fullname || !actor_id || !workcen_id || !password) {
...
const listUsers = async (req, res) => {
  try {
    ...
    if (search) {
      const total = await db.query(`SELECT COUNT(*) AS total FROM users WHERE npp ILIKE '%${search}%' OR fullname ILIKE '%${search}%'`)
...
const updateUser = async (req, res) => {
  try {
    const userID = req.params.id
    const { npp, nickname, fullname, actor_id, workcen_id } = req.body

    if (!npp || !nickname || !fullname || !actor_id || !workcen_id) return res.status(400).json({
      success: false,
      message: "Inputan tidak Lengkap !"
    })
    ...
    await db.query(
      `
        UPDATE users 
        SET 
          npp = $1, nickname = $2, fullname = $3, 
          actor_id = $4, workcen_id = $5, is_operator = $6
        WHERE user_id = $7
      `,
      [
        npp,
